...
const styles = {
  ...
  sectionSubtitle: { fontSize: 13, color: "#9ca3af", marginBottom: 24, marginTop: 0 },
...
export default function AddProductModal({ isOpen, onClose, onSubmit }) {
  ...
  // State riêng để nhập link ảnh hiện tại
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const addVariant = (opts, suffix) => {
        variants.push({
          price: price, stock: stock, sku: `${baseSku}-${suffix}`.toUpperCase(), options: opts,
        });
      };
      ...
      if (selectedSizes.length > 0 && selectedColors.length > 0) {
        ...
        variants.push({ price, stock, sku: baseSku, options: {} });
      }

      // 3. Chuẩn bị Images Payload (Quan trọng)
      let imagesPayload = [];
      if (formData.images.length > 0) {
          imagesPayload = formData.images.map((url, index) => ({
              url: url,
              isDefault: index === 0 
          }));
      } else {
          // Nếu không có ảnh nào, dùng ảnh placeholder
          imagesPayload = [{ url: "https://via.placeholder.com/300", isDefault: true }];
      }
      ...
      const payload = {
        ...
        images: imagesPayload, // Gửi mảng ảnh
        attributes: attributes,
        variants: variants,
      };

      // Gọi API
      await shopOwnerService.createProduct(payload);
      ...
      <div style={styles.modal}>
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", height: "100%" }}>
          <div style={styles.content}>
            ...
            <div style={styles.leftColumn}>
              ...
              <div style={styles.section}>
                ...
                <div style={styles.formRow}>
                  ...
                  <div style={styles.formGroup}>
                    <label style={styles.label}>Số lượng nhập</label>
            ...
            <div style={styles.rightColumn}>
              ...
              <div style={styles.section}>
                ...
                <p style={styles.sectionSubtitle}>Thêm URL ảnh (Ảnh đầu tiên sẽ là ảnh chính)</p>
                
                {/* Input nhập link + Nút Add */}
                <div style={styles.formGroup}>
                    <label style={styles.label}>Link ảnh URL</label>
                    <div style={styles.inputGroup}>
                        ...
                        />
                        <button type="button" style={styles.addImageBtn} onClick={handleAddImage}>
                    ...
                    <div style={styles.galleryGrid}>
                        {formData.images.map((url, index) => (
                            <div key={index} style={styles.imageWrapper}>
                                <img src={url} alt={`Product ${index}`} style={styles.thumbnail} 
                                     onError={(e) => e.target.src = "https://via.placeholder.com/150?text=Lỗi"}
                                />
                                {/* Nút xóa ảnh */}
                                <button type="button" style={styles.removeBtn} onClick={() => handleRemoveImage(index)}>
                                    <i className="fas fa-times"></i>
                                </button>
                                {/* Nhãn Default cho ảnh đầu tiên */}
                                {index === 0 && <div style={styles.defaultBadge}>Chính</div>}
                            </div>
                        ))}
                    </div>
                ) : (
              ...
              <div style={styles.section}>
                ...
                <div style={styles.colorRow}>
                  <div style={styles.colorItem}>
                    <input type="checkbox" style={styles.checkbox} checked={formData.colors.black} onChange={(e) => setFormData({ ...formData, colors: { ...formData.colors, black: e.target.checked } })} />
                    <div style={{ ...styles.colorCircle, background: "#000" }} />
                    <label style={styles.colorLabel}>Đen</label>
                  </div>
                  <div style={styles.colorItem}>
                    <input type="checkbox" style={styles.checkbox} checked={formData.colors.white} onChange={(e) => setFormData({ ...formData, colors: { ...formData.colors, white: e.target.checked } })} />
                    <div style={{ ...styles.colorCircle, background: "#fff" }} />
                    <label style={styles.colorLabel}>Trắng</label>
                  </div>
                  <div style={styles.colorItem}>
                    <input type="checkbox" style={styles.checkbox} checked={formData.colors.red} onChange={(e) => setFormData({ ...formData, colors: { ...formData.colors, red: e.target.checked } })} />
                    <div style={{ ...styles.colorCircle, background: "#dc2626" }} />
                    <label style={styles.colorLabel}>Đỏ</label>
                  </div>
                  <div style={styles.colorItem}>
                    <input type="checkbox" style={styles.checkbox} checked={formData.colors.blue} onChange={(e) => setFormData({ ...formData, colors: { ...formData.colors, blue: e.target.checked } })} />
                    <div style={{ ...styles.colorCircle, background: "#3b82f6" }} />
                    <label style={styles.colorLabel}>Xanh</label>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div style={styles.footer}>
            ...
            <button type="submit" style={styles.submitButton} disabled={isLoading}>{isLoading ? "Đang xử lý..." : "Thêm sản phẩm"}</button>
          </div>
        </form>
      </div>
    </div>
